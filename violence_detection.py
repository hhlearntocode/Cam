                    ...
                    if consecutive_violence_frames == 16 and violence_detected_info is None:
                        ...
                        for idx, frame_idx in enumerate(violence_frame_indices):
                            ...
                            if ok:
                                ...
                                if cv2.imwrite(violence_frame_path, violence_frame):
                                    violence_frames_list.append(violence_frame_path)
                        ...
                        violence_detected_info = {
                            ...
                            'violence_frames_list': violence_frames_list
                        }
                else:
                    # Reset consecutive violence frames if not a violence frame
                    if violence_detected_info is None:
                        consecutive_violence_frames = 0
                        violence_frame_indices.clear()
                        violence_frames_list.clear()
...
def main():
    ...
    result = detector.predict_frames("source/test1.mp4", "output.mp4") #video_path, khong de gi la dung webcam
    # result = detector.predict_frames() #video_path, khong de gi la dung webcam
    print(result)
    # Example usage of whole video prediction
    # print(detector.predict_video("source/fight.mp4"))
