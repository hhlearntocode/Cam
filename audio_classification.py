...
class audioProcess:
    ...
    def weight_audio_calculating(self, labels, scores):
        ...
        for i, label in enumerate(labels):
            if label in self.violence_labels_weight:
                print(
                    label,
                    self.violence_labels_weight[label],
                    tf.reduce_max(scores[i]).numpy(),
                )
                weight += (
                    self.violence_labels_weight[label]
                    * tf.reduce_max(scores[i]).numpy()
    ...
    def speech_to_text_from_waveform(self, waveform, sr=16000):
        ...
        transcription = self.voice_processor.batch_decode(
    ...
    def analyze_text_audio(self, text, labels):
        detected_keywords = [
            word for word in self.violence_keywords_weight if word in text
        ]
        detected_sound = [
            sound for sound in self.violence_labels_weight if sound in labels
        ]
    ...
    def analyze_violence_rate(self, labels, scores, text):
        ...
        if weight_audio > 0 and weight_keywords == 0:
            ...
            if any(
                label
                in ["Slap, smack", "Breaking", "Glass", "Wail, moan", "Smash, crash"]
                for label in labels
            ):
                print(
                    "Có dấu hiệu hành vi bạo hành trẻ em, xin vui lòng kiểm tra camera"
                )
        ...
        max_audio_weight = max(
            self.violence_labels_weight.values()
        )  # Trọng số lớn nhất của âm thanh
        max_keyword_weight = max(
            self.violence_keywords_weight.values()
        )  # Trọng số lớn nhất của từ khóa
        normalized_score = total_weight / (max_audio_weight + max_keyword_weight)

        if normalized_score > 0.2:
            return {"label": "Violence", "confidence_score": float(normalized_score)}
        else:
            return {"label": "NonViolence", "confidence_score": float(normalized_score)}
    ...
    def record_and_predict(self, duration=5, sr=16000, device_port=1):
        ...
        for i in predictions.numpy():
            class_name = str(
                self.labels[self.labels["index"] == i]["display_name"].values[0]
            )
